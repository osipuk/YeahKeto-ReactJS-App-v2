...
const pUpsell = (function () {
  return {
    ...
    _getNextPageUrl() {
      if (window.location.pathname.indexOf('/upsell.h') >= 0) {
        return 'upsell-2.html' + window.location.search;
      } else if (window.location.pathname.indexOf('/upsell-2') >= 0) {
        return 'upsell-3.html' + window.location.search;
      }
    ...
    _onUpsellYes() {
      ...
      $('.upsell-link').click(function (e) {
        ...
        usParams.productId = $(this).data('upsell');
         ...
         if (usParams.productId >= 168 && usParams.productId <= 174) {
          const nextPage = $parent._getNextPageUrl();
